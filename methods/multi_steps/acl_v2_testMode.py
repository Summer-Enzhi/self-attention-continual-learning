...
class ACL(Finetune_IL):
    ...
    def prepare_model(self, checkpoint=None):
        ...
        self.select_fc = nn.Linear(512, 11).cuda()
    ...
    def incremental_train(self):
        ...
        if self._cur_task<2:
            ...
            for params in self._network.select_fc.parameters():
            ...
            # best_id = self._select_model(self._network, self._train_loader, self._test_loader,
            #     task_id=self._cur_task, epochs=self._epochs, note='stage1')
            # print(f'=====select_model_init=====\nBest_id:{best_id} in task {self._cur_task}')
            # self._network.select_model_init(best_id)
            
            
            self._network.unfreezeLast()
            self._network.train_adapter_mode()
            ...
            self._network = self._train_model(self._network, self._train_loader, self._test_loader, optimizer, scheduler,
                task_id=self._cur_task, epochs=self._epochs, note='stage1',best_id = 0)
            self._network.seperate_fc[self._cur_task].load_state_dict(self._network.aux_fc.state_dict())
    ...
    def _train_model(self, model, train_loader, test_loader, optimizer, scheduler, task_id=None, epochs=100, note='', best_id = 0):
        ...
        for epoch in range(epochs):
            model, train_acc, train_losses = self._epoch_train(model, train_loader, optimizer, scheduler,
                                task_begin=task_begin, task_end=task_end, task_id=task_id, best_id = best_id)
            # update record_dict
            record_dict = {}
    ...
    def _select_model(self, model, train_loader, test_loader, task_id=None, epochs=100, note=''):
        auc_list = []
        old_state = copy.deepcopy(self.select_fc.state_dict())
        for t in range(0,task_id):
            task_begin = sum(self._increment_steps[:t])
            task_end = task_begin + self._increment_steps[t]
            optimizer = self._get_optimizer(self.select_fc.parameters(), self._config, False)
            scheduler = self._get_scheduler(optimizer, self._config, False)
            _, train_acc, train_losses = self._epoch_select(model, train_loader, optimizer, scheduler,
                                task_begin=task_begin, task_end=task_end, task_id=task_id, select = t)
            self.select_fc.load_state_dict(old_state)
            auc_list.append(train_acc)
            
        print(f'auc_list:{auc_list}')
        return auc_list.index(max(auc_list))
    
    def _epoch_train(self, model, train_loader, optimizer, scheduler, task_begin=None, task_end=None, task_id=None,best_id = None):
        correct, total = 0, 0
        ce_losses = 0.
        losses = 0.
        ...
        for _, inputs, targets in tqdm(train_loader):
            ...
            if best_id:
                logits, output_features = model(inputs,select_adapter = best_id)
            else: 
    ...
    def _epoch_select(self, model, train_loader, optimizer, scheduler, task_begin=None, task_end=None, task_id=None, select = None):
        correct, total = 0, 0
        ce_losses = 0.
        losses = 0.
        th = 0.5 if task_id==0 else 0.1
        ...
        for _, inputs, targets in tqdm(train_loader):
            ...
            # model forward has two mode which shoule be noticed before forward!
            with torch.no_grad():
                _, output_features = model(inputs,select_adapter = select)
            logits = self.select_fc(output_features['features'][select])
            if num_classes==0:
                num_classes = logits[:,:-1].shape[-1]
                correct0 = [0] * num_classes
                total0 = [0] * num_classes     
