...
class DataManager(object):
    def __init__(self, logger, dataset_name, img_size, split_dataset:bool, shuffle=False, seed=0, init_cls=None, increment=None, use_valid=False):
        ...
        if hasattr(idata, '_dataset_inc'):  # for datasets which have sub-datasets, likes MedMNistS
            ...
            if shuffle:
                np.random.seed(seed)
                shuffled_indices = np.random.permutation(len(order[1:]))
                order[1:] = np.array(order)[1:][shuffled_indices].tolist()
    ...
    def get_dataset(self, source, mode, indices=[], appendent=[], ret_data=False, two_view=False,increment_steps = None,openset = None):
        """
        作用: 获取指定类别范围的数据
        ...
        data, targets = [], []
        # for idx in indices:
        class_data, class_targets = self._select(x, y, low_range=indices[0], high_range=indices[-1]+1,increment_steps = increment_steps,openset=openset)
    ...
    def _select(self, x, y, low_range, high_range, soft_y=None, increment_steps = None, openset = None):
        ...
        idxes = y[:, low_range] != 2
        x_ = x[idxes]
        y_ = y[idxes][:,low_range:low_range+1]
        if increment_steps and low_range > 0:
            if not openset:
                # start = int(((low_range-1) / sum(increment_steps[1:]))*x_.shape[0])
                start = 0
                end = int(((low_range) / sum(increment_steps[1:]))*x_.shape[0])
            else:
                start = 0
                end = int(((openset-increment_steps[0]) / sum(increment_steps[1:]))*x_.shape[0])
