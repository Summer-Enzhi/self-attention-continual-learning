...
def get_backbone(logger, backbone_type, pretrained=False, pretrain_path=None, normed=False) -> nn.Module:
    ...
    if pretrain_path is not None and pretrained:
        ...
        print("loaded pretrained weights from default")
...
class IncrementalNet(nn.Module):
    ...
    def __init__(self, logger, backbone_type, pretrained, pretrain_path=None, layer_names: Iterable[str]=[], MLP_projector=False,rig = True):
        ...
        print("Removed original backbone--{}'s fc classifier !".format(backbone_type))
        # print("Removed original backbone--{}'s fc classifier !".format(backbone_type))
        self.fc = None
        if MLP_projector:
    ...
    def set_hooks(self):
        ...
        if len(self._layer_names)>0:
            ...
            for layer_id in self._layer_names:
                ...
                print('Registered forward hook for {}'.format(layer_id))
        # return self
    ...
    def forward(self, x, select_id = None):
        features = self.feature_extractor(x)
        ...
        if len(self.seperate_fc) == 0 and self.fc is not None:
            ...
            if not select_id:
                out = self.seperate_fc[-1](features)
                if isinstance(out, tuple):
                    # for special classifier head which return (Tensor, Dict), e.g. cosineLinear
                    self.output_features.update(out[1])
                    out = out[0]
            else:
                out = self.seperate_fc[select_id](features)
    ...
    def update_fc(self, nb_classes):
        ...
        fc = self.generate_fc(self.feature_dim, self.task_sizes[-1])
        # fc = self.generate_fc(self.feature_dim, nb_classes)
        # if self.fc is not None:
        if len(self.task_sizes) > 1:
            nb_output = self.seperate_fc[-1].out_features
            weight = copy.deepcopy(self.seperate_fc[-1].weight.data)
            bias = copy.deepcopy(self.seperate_fc[-1].bias.data)
            fc.weight.data[:nb_output] = weight
            fc.bias.data[:nb_output] = bias
            print('Updated classifier head output dim from {} to {}'.format(nb_output, nb_classes))
        else:
            print('Created classifier head with output dim {}'.format(nb_classes))
        # del self.fc
        # self.fc = fc
        self.seperate_fc.append(fc)
    ...
    def copy(self):
        ...
        print("Setting copied network's hooks...")
        copy_net.set_hooks()
        print("Reseting current network's hooks...")
        self.set_hooks()
        return copy_net
    ...
    def freeze_FE(self):
        ...
        print('Freezing feature extractor(requires_grad=False) ...')
        return self
    ...
    def activate_FE(self):
        ...
        print('Activating feature extractor(requires_grad=True) ...')
        return self
    ...
    def freeze(self):
        for param in self.parameters():
            param.requires_grad = False
        self.eval()
        print('Freezing the whole network(requires_grad=False) ...')
        return self
    ...
    def activate(self):
        for param in self.parameters():
            param.requires_grad = True
        self.train()
        print('Activating the whole network(requires_grad=True) ...')
        return self
    ...
    def weight_align(self, increment):
        ...
        newnorm = (torch.norm(self.seperate_fc[-1].weight.data,p=2,dim=1))
        meannew = torch.mean(newnorm)
        for old in range(len(self.seperate_fc)-1):
            oldnorm = (torch.norm(self.seperate_fc[old].weight.data,p=2,dim=1))
            meanold = torch.mean(oldnorm)
            gamma = meanold/meannew
            print('alignweights,gamma=',gamma)
            self.seperate_fc[old].weight.data *= gamma
...
class IncrementalNet2(nn.Module):

    def __init__(self, logger, backbone_type, pretrained, pretrain_path=None, layer_names: Iterable[str]=[], MLP_projector=False,rig = True):
        super(IncrementalNet2, self).__init__()
        '''
        layers_name can be ['conv1','layer1','layer2','layer3','layer4']
        '''
        self._logger = logger
